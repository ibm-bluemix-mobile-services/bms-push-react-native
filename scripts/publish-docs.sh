...
docs_directory='./ibm-bluemix-mobile-services.github.io/API-docs/client-SDK/Push-React-Native'
rm -rf "${docs_directory}"/*
jsdoc -c config.json -d "${docs_directory}"
mv "${docs_directory}"/bmd-push-react-native/"${version}"/* "${docs_directory}"

# Publish docs
cd ibm-bluemix-mobile-services.github.io
git add .
git commit -m "Published docs for Push react-native plugin ${version}"
git rebase master
